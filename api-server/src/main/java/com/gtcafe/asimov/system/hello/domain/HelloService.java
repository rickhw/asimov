...
public class HelloService {
  ...
  public HelloEvent sayHelloAsync(Hello hello) {
    ...
    try {
      ...
      log.debug("Stored hello event to cache with keys: {} and {}", cachedKey, taskCachedKeyForIndex);
      ...
      log.info("Successfully processed asynchronous hello request with event ID: {}", event.getId());
      return event;
      
    } catch (Exception e) {
      log.error("Error processing asynchronous hello request for message: {}", hello.getMessage(), e);
